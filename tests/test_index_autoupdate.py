...
def test_add_simple():
    ...
    assert interface["sessions"].filter(state="new") == {interface["sessions"]["f"]}
...
def test_remove_simple():
    ...
    assert interface["sessions"].filter(state="new") == {interface["sessions"]["c"]}
...
def test_remove_complex():
    ...
    assert interface.indexes["sessions"]["__index__"]["state"]() == {
        state: {} for state in ["new", "processed", "erroneous"]
    }
...
def test_update_value():
    ...
    interface["sessions"] = {"a": {"state": "new"}}
