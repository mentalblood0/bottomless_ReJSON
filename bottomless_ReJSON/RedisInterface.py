...
class RedisInterface:
    ...
    def updateIndexesList(self):
        ...
        while True:
            result = [e for r in result for e in list(r) if not "__index__" in r.path]
    ...
    def filter(self, **kwargs):
        ...
        for field, value in kwargs.items():
            if len(keys) < len(self.getIndex(field)[value]):
                keys = {k for k in keys if self.getIndex(field)[value][k].type != None}
            else:
    ...
    def __len__(self):
        ...
        d = {"object": "jsonobjlen", "array": "jsonarrlen"}
